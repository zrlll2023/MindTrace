<template>
  <div class="page me-page">
    ...
    <section class="card">
      ...
      <div class="range"><input v-model="dateFrom" type="date" @change="loadMetrics" /><span>至</span><input v-model="dateTo" type="date" @change="loadMetrics" /><button class="secondary small" @click="last30">最近 30 天</button></div>
      ...
      <div v-if="metrics.length" class="trend-list"><div v-for="m in metrics" :key="m.date" class="trend-row"><time>{{ m.date }}</time><span>睡眠 {{ m.sleep_hours == null ? '—' : `${m.sleep_hours}h` }}</span><span>负面事件 {{ m.negative_count }}</span><span>记录 {{ m.entry_count }}</span></div></div>
...
interface Metric{date:string;sleep_hours:number|null;negative_count:number;entry_count:number}
...
<style scoped>
.me-page{max-width:900px}.section-title{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.section-title h3{margin:0}.profile-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.field.wide{grid-column:1/-1}.source{font-size:10px;color:var(--accent-text);background:var(--accent-weak);padding:1px 5px;border-radius:var(--r-sm)}.range{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin:12px 0}.range input{width:auto}.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:12px 0}.stat-row div{padding:14px;background:var(--surface-2);border-radius:var(--r);display:flex;flex-direction:column}.stat-row b{font-size:24px}.stat-row span{font-size:12px;color:var(--text-3)}.trend-list{max-height:320px;overflow:auto}.trend-row{display:grid;grid-template-columns:1.2fr repeat(3,1fr);gap:8px;padding:8px 4px;border-bottom:1px solid var(--border);font-size:12.5px}.trend-row time{font-weight:600}@media(max-width:700px){.profile-grid{grid-template-columns:1fr}.stat-row{grid-template-columns:1fr}.trend-row{grid-template-columns:1fr 1fr}}
</style>
