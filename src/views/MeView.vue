<template>
  <div class="page me-page">
    ...
    <section class="card">
      <div class="section-title"><div><h3>生活趋势</h3><p class="hint">睡眠时长与负面事件按日回顾，仅供自我观察。</p></div></div>
      ...
      <div v-if="metrics.length" class="trend-list"><div v-for="m in metrics" :key="m.date" class="trend-row"><time>{{ m.date }}</time><span>睡眠 {{ m.sleep_hours == null ? '—' : `${m.sleep_hours}h` }}</span><span>{{ sleepDetail(m) }}</span><span>负面事件 {{ m.negative_count }}</span><span>记录 {{ m.entry_count }}</span></div></div>
      <div v-else class="empty">该区间暂无记录。</div>
    </section>
  </div>
</template>
<script setup lang="ts">
import { computed, onMounted, reactive, ref } from 'vue'
import type { ProfileKey, ProfileValues } from '../../electron/types'
...
interface Metric{date:string;sleep_hours:number|null;negative_count:number;entry_count:number;sleep_sessions:number;longest_sleep_hours:number|null;sleep_data_mode:'sessions'|'daily_total'|'duration_only'|'none'}
const metrics=ref<Metric[]>([]),today=()=>new Date().toISOString().slice(0,10),ago30=()=>{const d=new Date();d.setDate(d.getDate()-29);return d.toISOString().slice(0,10)},dateFrom=ref(ago30()),dateTo=ref(today())
const avgSleep=computed(()=>{const a=metrics.value.filter(m=>m.sleep_hours!=null).map(m=>m.sleep_hours!);return a.length?(a.reduce((x,y)=>x+y,0)/a.length).toFixed(1):'—'}),totalNegative=computed(()=>metrics.value.reduce((n,m)=>n+m.negative_count,0)),totalEntries=computed(()=>metrics.value.reduce((n,m)=>n+m.entry_count,0))
function sleepDetail(m:Metric){if(m.sleep_data_mode==='sessions')return `${m.sleep_sessions} 段 · 最长 ${m.longest_sleep_hours}h`;if(m.sleep_data_mode==='daily_total')return '当天累计 · 分段未知';if(m.sleep_data_mode==='duration_only')return '旧记录 · 分段未知';return '无睡眠记录'}
...
<style scoped>
.me-page{max-width:900px}.section-title{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.section-title h3{margin:0}.profile-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.field.wide{grid-column:1/-1}.source{font-size:10px;color:var(--accent-text);background:var(--accent-weak);padding:1px 5px;border-radius:var(--r-sm)}.range{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin:12px 0}.range :deep(.dp--main){width:150px}.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:12px 0}.stat-row div{padding:14px;background:var(--surface-2);border-radius:var(--r);display:flex;flex-direction:column}.stat-row b{font-size:24px}.stat-row span{font-size:12px;color:var(--text-3)}.trend-list{max-height:320px;overflow:auto}.trend-row{display:grid;grid-template-columns:1.1fr repeat(4,1fr);gap:8px;padding:8px 4px;border-bottom:1px solid var(--border);font-size:12.5px}.trend-row time{font-weight:600}@media(max-width:700px){.profile-grid{grid-template-columns:1fr}.stat-row{grid-template-columns:1fr}.trend-row{grid-template-columns:1fr 1fr}}
</style>
