...
<style scoped>
.me-page{max-width:900px}.section-title{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.section-title h3{margin:0}.profile-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.field.wide{grid-column:1/-1}.source{font-size:10px;color:var(--accent-text);background:var(--accent-weak);padding:1px 5px;border-radius:var(--r-sm)}.range{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin:12px 0}.range :deep(.dp--main){width:150px}.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:12px 0}.stat-row div{padding:14px;background:var(--surface-2);border-radius:var(--r);display:flex;flex-direction:column}.stat-row b{font-size:24px}.stat-row span{font-size:12px;color:var(--text-3)}.trend-list{max-height:320px;overflow:auto}.trend-row{display:grid;grid-template-columns:1.2fr repeat(3,1fr);gap:8px;padding:8px 4px;border-bottom:1px solid var(--border);font-size:12.5px}.trend-row time{font-weight:600}@media(max-width:700px){.profile-grid{grid-template-columns:1fr}.stat-row{grid-template-columns:1fr}.trend-row{grid-template-columns:1fr 1fr}}
</style>
